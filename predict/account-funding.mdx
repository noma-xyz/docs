...
| If you… | Use | Fee | Time |
|---|---|---|---|
| Already hold USDC in a wallet | **Wallet-to-wallet transfer** | Source-chain gas only | ~30 seconds |
| Want to pay with a debit card or bank | **Coinbase Onramp** (fiat on-ramp) | Coinbase's standard card/bank fee | ~2 minutes |
| Hold crypto on Ethereum, Arbitrum, Base, or another chain | **Blink** (bridge) | **0%** — Blink sponsors bridge fees | ~1–3 minutes |
...
### Base — USDC (for Limitless markets)

Limitless Exchange markets settle in native **USDC** on Base (Ethereum L2). Gas is fully sponsored — you never need Base ETH.

- **Network:** Base
- **Token:** USDC
- **Deposit address:** Your Base wallet address — shown in the deposit modal inside the app

### How to do it

1. In the noma app, click the **+** button in the header to open the deposit modal
2. Pick the network you want to fund — **Polygon** (USDC.e), **Solana** (USDC), or **Base** (USDC)
3. Copy your deposit address, or scan the QR code with your wallet app
4. Send from your external wallet
5. Funds usually confirm in under 30 seconds on Polygon, a few seconds on Solana, and a few seconds on Base
...
Once funds are in noma, no — noma covers gas on Polygon, Solana, and Base for every trade, order cancellation, and payout claim. For the deposit itself, you pay gas on the source chain you're sending from (Ethereum, Arbitrum, etc.) — that's a network fee paid to validators, not to noma or Blink.
...
**Do Polygon, Solana, and Base balances sync?**
No — they're separate. USDC.e on Polygon funds Polymarket markets; USDC on Solana funds Kalshi; USDC on Base funds Limitless. noma picks the right chain automatically when you trade, and the UI shows your total balance across all three.
